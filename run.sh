...
docker rm -f nuaactf_tinyctf
docker run -d --name=nuaactf_tinyctf --net=host --restart=always -v $DIR/web:/app/web -v $DIR/tinyctf-platform:/app/tinyctf nuaactf-python bash -c "/app/tinyctf/bootstrap.sh"
...
docker run -d --name=nuaactf_php-fpm --net=host --restart=always -v $DIR/web:/app/web php:fpm
