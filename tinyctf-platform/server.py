...
def get_task(category, row):
    """Finds a task with a given category and row"""

    task = db.query('''select t.* from tasks t, categories c, cat_task ct 
        where t.id = ct.task_id and c.id = ct.cat_id 
        and t.row=:row and lower(c.short_name)=:cat and t.show = 1''',
        row=row, cat=category)
    return list(task)[0]

def get_bonus(task):
    """calculate bonus"""
    flags = db.query('''select count(*) count from flags where task_id = :taskid''',
        taskid=task['id'])
    correct_num = int(list(flags)[0]['count'])

    if int(task['score']) < 100 :
        if correct_num == 0:
            return 15
        if correct_num == 1:
            return 10
...
def tasks():
    ...
    tasks = db.query('''select c.id as cat_id, t.id as id, c.short_name, 
        ...
        where c.id = c_t.cat_id and t.id = c_t.task_id and t.show = 1''')
    tasks = list(tasks)
...
@app.route('/tasks/<category>/<row>')
@login_required
def task(category, row):
    """Displays a task with a given category and row"""

    login, user = get_user()

    task = get_task(category, row)
    if not task:
    ...
    render = render_template('frame.html', lang=lang, page='task.html', 
        ...
        user=user, category=category, task=task, row=row, score=task['score'])
    return make_response(render)

@app.route('/submit/<category>/<row>/<flag>')
@login_required
